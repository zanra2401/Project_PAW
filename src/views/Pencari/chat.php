    ...
    <body class="overflow-hidden flex p-0 m-0">
        <main class="flex flex-1">
            <div class="h-screen flex flex-1 bg-gray-100 mt-1">
                ...
                <div id="contact-sidebar" class="w-1/2 bg-white  border-r flex flex-col items-center border-gray-200 overflow-y-auto">
                    <!-- Header -->
                    <div class="p-4 border-b border-gray-200 flex w-full items-center justify-start shadow-sm">
                    ...
                    <div id="contact-list" class="p-2 w-[99%] ">    
                        ...
                        <?php
                            foreach ($data['contact'] as $contact) {
                                ...
                                echo <<<EOD
                                /pencari/chatting/{$contact[0]['id_user']}" class="flex relative items-center group space-x-3 rounded-md p-4 hover:bg-gray-100 cursor-pointer">
                    ...
                    .then(data => {
                        ...
                        data.forEach(contact => {
                            contacts += `<a href="/<?= PROJECT_NAME ?>/pencari/chatting/${contact[0]['id_user']}" class="flex relative items-center group space-x-3 rounded-md p-4 hover:bg-gray-100 cursor-pointer">
                                    <img src="/<?= PROJECT_NAME ?>/${contact[0]['profile_user']}" alt="Profile" class="w-10 h-10 object-cover rounded-full">
                                    <div class="flex-1">
                                        <h3 class="text-lg font-medium ">${contact[0]['username_user']}</h3>
                                    </div>`
                            if (contact['unread'] > 0){
