...
$this->DB->query("SELECT * FROM kost WHERE (nama_kost LIKE '%$nama_lokasi%' OR kota_kost LIKE '%$nama_lokasi%' OR
provinsi_kost LIKE '%$nama_lokasi%') AND tipe_kost = '$tipe'");
...
$this->DB->query("SELECT COUNT(*) AS total_kamar FROM kamar WHERE kamar.id_kost = {$kost['id_kost']} AND
...
$this->DB->query("SELECT tf.nama_fasilitas, tf.tipe_fasilitas FROM fasilitas_kost AS f, fasilitas AS tf WHERE
f.id_fasilitas = tf.id_fasilitas AND f.id_kost = {$id_kost}");
...
{
$data = [];
$this->DB->query("SELECT username_user, email_user, nama_user, no_HP_user, jenis_kelamin_user, email_user, no_HP_user,
provinsi_user, kota_user, profile_user FROM user WHERE id_user = $id");
...
$this->DB->query("UPDATE user SET username_user = '$username', email_user = '$email', nama_user = '$nama_lengkap',
no_HP_user = '$no_hp', jenis_kelamin_user = '$jenis_kelamin', provinsi_user = $provinsi, kota_user = $kota, profile_user
= '$pathName' WHERE id_user = $id");
} else {
$this->DB->query("UPDATE user SET username_user = '$username', email_user = '$email', nama_user = '$nama_lengkap',
no_HP_user = '$no_hp', jenis_kelamin_user = '$jenis_kelamin', provinsi_user = $provinsi, kota_user = $kota WHERE id_user
= $id");
}
...
$this->DB->query("SELECT u.username_user, u.id_user FROM user AS u, kost AS k WHERE k.id_user = u.id_user AND id_kost =
{$id}");
$result = $this->DB->getAll();
...
$this->DB->query("SELECT * FROM kost WHERE (kota_kost = (SELECT kota_kost FROM kost WHERE id_kost = $id) OR
provinsi_kost = (SELECT provinsi_kost FROM kost WHERE id_kost = $id)) AND id_kost != $id LIMIT 5;");
$kosts = $this->DB->getAll();
...
$this->DB->query("INSERT INTO laporan (id_user, deskripsi_laporan, id_kost) VALUES (?,?,?)", "isi",
[$id_user,$detail,$id_kost]);
...
$this->DB->query("INSERT INTO laporantokategori (id_laporan, id_kategori_laporan) VALUE (?,?)", "ii", [$data,
$id_kategori]);
}
}
...
{
$this->DB->query("SELECT COUNT(*) AS jumlah FROM favorit WHERE id_user = ? AND id_kost = ?", "ii", [$id_user,
$id_kost]);
$result = $this->DB->getAll();
$data = $result[0]['jumlah'];
if ($data === 0){
...
{
$data = [];
$this->DB->query("SELECT kl.tanggal_review, kl.isi_review, u.profile_user, u.username_user FROM review AS kl, user AS u
WHERE kl.id_kost = $id_kost AND kl.id_user = u.id_user");
$result = $this->DB->getAll();
foreach ($result as $res)
{
$data[] = $res;
...
if ($noUpdate)
{
$this->DB->query("UPDATE chat SET status_chat = 1 WHERE id_user_penerima = ? AND id_user_pengirim = ?", "ii",
[$_SESSION['id_user'], $idPengirim]);
}
$this->DB->query("SELECT * FROM chat WHERE (id_user_penerima = ? AND id_user_pengirim = ?) OR (id_user_penerima = ? AND
id_user_pengirim = ?) ORDER BY tanggal_chat ASC", 'iiii', [$_SESSION['id_user'], $idPengirim, $idPengirim,
$_SESSION['id_user']]);
...
$this->DB->query("INSERT INTO chat(id_user_pengirim, id_user_penerima, isi_chat, tanggal_chat, status_chat) VALUES(?, ?,
?, ?, 0)", "iiss", [$_SESSION['id_user'], $idPenerima, $message, $current_datetime]);
return true;
}
...
{
$data = [];
$this->DB->query("SELECT DISTINCT u.id_user, u.username_user, u.profile_user FROM user u JOIN chat c ON u.id_user =
c.id_user_pengirim WHERE c.id_user_penerima = ?;", "i", [$_SESSION['id_user']]);
